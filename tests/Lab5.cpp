...
class CommandPromptTest : public CommandPrompt {
public:
    using CommandPrompt::listCommands; // Make the protected method public in the subclass
    using CommandPrompt::prompt; // Make the protected method public in the subclass
};
//new additions to ensure that protected methods are able to be tested.
//imports the definition from the base class into the test class.
...
TEST(commandPrompt,addAndListCommands){ // ensures the add command and list command are working correctly, we would expect the command name for the command we added (in this case the testCommand) to be printed out by the listCommand() method
    // SET UP FILE SYSTEM
    AbstractFileSystem* sfs = new SimpleFileSystem();
    AbstractFileFactory* sff = new SimpleFileFactory();
    CommandPromptTest cp;
    ...
    ASSERT_EQ(1, static_cast<int>(printedWords.size()));
    ASSERT_EQ(commandname, printedWords[0]);
    // ASSIGN COUT BACK TO STDOUT
    cout.rdbuf(backup);
}

TEST(commandPrompt,prompt) { // checks prompt method, ensures that prompt returns what is typed in the terminal by the filesystem user
    // SET UP FILE SYSTEM
    AbstractFileSystem* sfs = new SimpleFileSystem();
    AbstractFileFactory* sff = new SimpleFileFactory();
    CommandPromptTest* cp = new CommandPromptTest;
    cp->setFileSystem(sfs);
    cp->setFileFactory(sff);
    // REDIRECT COUT STREAM
    streambuf* backup_out;
    ...
    ASSERT_EQ(input, promptResponse);
    // ASSIGN COUT BACK TO STDOUT
    cout.rdbuf(backup_out);
    // ASSIGN CIN BACK TO STDIN
    cin.rdbuf(backup_in);
